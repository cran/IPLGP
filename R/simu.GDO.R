...
#' }
simu.GDO <- function(fittedA.t, fittedD.t = NULL, fittedmu.t =NULL, geno.t, marker, geno.c = NULL, npl = NULL,
                     better.c = FALSE, weight = NULL, direction = NULL, outcross = FALSE, nprog = 50,
                     nsele = NULL, ngen = 10, nrep = 30, cri = 10000, console = TRUE){
  ...
  if(better.c){
    ...
    for(i in 1:nt){
      ...
      sele <- cbind(sele, sele1)
    }
    sele <- apply(sele, 1, prod)
    fit.c.better <- p.c
    fit.c.better <- row.names(fit.c.better)[sele == 1]
  ...
  for(m in 1:nrep){
    ...
    GDO.p0 <- cbind(1:nrow(phe.p0), phe.p0+phe.p0D, geno.p0)
    ...
    GDO.gvalue[[1]] <- GEBV0*matrix(sd0, nrow(GDO.p0), nt, byrow = TRUE)+
      GEBV0D*matrix(sdD, nrow(GDO.p0), nt, byrow = TRUE)+matrix(mu0, nrow(GDO.p0), nt, byrow = TRUE)
    ...
    if(outcross){
      ...
      Kpt <- t(geno.t%*%t(GDO.F1.SNP2))/ncol(GDO.F1.SNP2)
      KptD <- Kpt
      KptD[] <- 0
    }
    ...
    GDO.gvalue[[2]] <- fity.F1*matrix(sd0,nrow(fity.F1), nt, byrow = TRUE)+
    ...
    if(outcross){
      ...
      for(i in 1:(nf1*nprog)){
        ...
        GDO.F2.SNP <- cbind(GDO.F2.SNP, F2_0)
      }

      GDO.SNP[[3]] <- GDO.F2.SNP
      GDO.F2.SNP2 <- rbind(geno.t, t(GDO.F2.SNP))
      gd.F2.SNP2 <- geno.d(GDO.F2.SNP2)
      ...
      GDO.gvalue[[3]] <- fity.F2*matrix(sd0, nrow(fity.F2), nt, byrow = TRUE)+
        fity.F2D*matrix(sdD, nrow(fity.F2), nt, byrow = TRUE)+matrix(mu0, nrow(fity.F2), nt, byrow = TRUE)

      if(console){cat("GDO", m, paste("F", 2, sep = ""), "\n", sep = "\t")}
    ...
    for(g in g0:(ngen+1)){
      ...
      if(outcross){
        ...
        Kpt2 <- t(geno.t%*%t(GDO.F2.SNP2))/ncol(GDO.F2.SNP2)
        Kpt2D <- Kpt2
        Kpt2D[] <- 0
      }
      ...
      GDO.gvalue[[g]] <- fity.F2*matrix(sd0, nrow(fity.F2), nt, byrow = TRUE)+
        fity.F2D*matrix(sdD, nrow(fity.F2), nt, byrow = TRUE)+matrix(mu0, nrow(fity.F2), nt, byrow = TRUE)
  ...
  return(list(method = "GD-O", weight = weight, direction = direction, mu = phe1[[2]], sd = sd0,
